...
If the light was flashing but has since gone steady or turned off, the misfire may have been intermittent. The fault code will still be stored in the ECM. Schedule before the code clears - [advanced diagnostics](/services/advanced-diagnostics) can pull stored fault history and freeze frame data that identifies the root cause even after the symptom stops presenting.
